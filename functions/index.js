 ...
 */
exports.grantAdminRole = onRequest(async (req, res) => {
  // 设置 CORS 跨域请求头
  ...
  // 验证请求头中的 Firebase ID Token
  if (!req.headers.authorization || !req.headers.authorization.startsWith("Bearer ")) {
    logger.error("未在 Authorization 请求头中传递 Firebase ID token。");
  ...
  try {
    ...
    if (decodedToken.role !== "admin") {
      logger.error("调用者不是管理员，权限不足。");
      res.status(403).send("Unauthorized");
      return;
    }
  } catch (error) {
    logger.error("验证 Firebase ID token 时出错:", error);
    res.status(403).send("Unauthorized");
    return;
  }
 ...
 * 处理点击追踪的 HTTP 云函数 (V2)
 */
exports.trackClick = onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
